...
RSpec.describe API::V1::SessionResource do
  describe 'POST /api/v1/session.json' do
    let(:uri) { '/api/v1/session.json' }
    let(:request_params) { {} }
    let(:headers_params) { {} }
    ...
    context 'email with password' do
      ...
      before do
        ActiveRecord::Base.transaction do
          user = User.create!(request_params[:user].merge(state: 'active'))

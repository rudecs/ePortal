...
namespace :cron do
  desc 'Create resourse\'s usage for last hour'
  task hourly_reports: :environment do
    log = ActiveSupport::Logger.new('log/cron_hourly_reports.log')
    ...
    Resource.active.where.not(id: r_ids).find_each do |res|
      ...
      if Services::Payloads::Create.new(res, fake_event, billing_period: [period_start, period_end]).call # check: save method inside. Cause save retuns record.
        ...
        log.info "FAILED: resource_id = #{res.id}"
      end
    end

    task_end_time = Time.current
    task_duration = task_end_time - task_start_time
    log.info "=== Task finished at #{task_end_time}. #{payloads_created_count} payloads created. It took #{task_duration} seconds. ==="
    log.close
  end
end

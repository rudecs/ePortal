module API
  class V1::WriteoffsResource < API::V1
    ...
    resource :writeoffs, desc: 'Cписания' do
      ...
      get :show, jbuilder: 'writeoffs/show.json' do
        ...
        @writeoffs = Writeoff.paid.where(client_id: params[:client_id], end_date: start_at..end_at).includes(:product_instance_states).order(end_date: :desc).page(params[:page]).per_page(params[:per_page])
      end
    end
  end
end
